...
#### 3.2.1使用Spring Initializr 的 Web页面创建项目
...
#### 3.2.5将项目打成jar包，点击 maven的 package

遇到错误，可以配置打包时 跳过项目运行测试用例
...
Springboot所有自动配置都是在启动的时候扫描并加载；所有的自动配置都在spring.factories，但是不一定生效，要判断条件是否成立，只要导入了对应的start，就有对应的启动器了，有了启动器，自动装配就会生效，然后就配置成功！
...
6. 容器中也会存在非常多的xxxAtuoConfiguration的文件（@Bean），就是这些类给容器中导入了这个场景需要的所有组件并自动配置，@Configuration,JavaConfig
...
```
#配置项目的访问路径
server.servlet.context-path=/shen
```
...
2、新建一个普通Maven模块：shen-spring-boot-starter

3、新建一个Springboot模块：shen-spring-boot-starter-autoconfigure
